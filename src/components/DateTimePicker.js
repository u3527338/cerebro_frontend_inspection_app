                ...
                render={
                  ({field: {onChange, onBlur, value}}) => (
                    <VStack space={1}>
                      ...
                      <HStack justifyContent={"space-between"} alignItems={"center"} mb={3} space={4}>
                        <Pressable onPress={showDatepicker} flex={2}>
                          <Box borderRadius={4} borderWidth={1} py={2}
                               bg={"coolGray.50"}
                               borderColor={"coolGray.400"}
                          >
                            <Text color={"coolGray.800"} textAlign={"center"}>
                              {(value) ? `${value.getDate()} / ${value.getMonth() + 1} / ${value.getFullYear()}` : null}
                            </Text>
                          </Box>
                        </Pressable>
                        <Pressable onPress={showTimepicker} flex={1}>
                          ...
                          >
                            <Text textAlign={"center"} color={"coolGray.800"}>
                              {(value) ? `${value.getHours()} : ${(value.getMinutes() < 10 ? '0' : '') + value.getMinutes()}` : null}
                            </Text>
                          </Box>
                        </Pressable>
                      </HStack>
