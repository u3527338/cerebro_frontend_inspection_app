...
export default function StackedBarChartExample({ data }) {
  ...
  const maxData = Math.max(
    ...data.map((value, index) =>
      keys.map((key) => value[key]).reduce((a, b) => a + b)
    )
  );
  const x_label = Array.from(
    { length: 6 },
    (x, i) => (i * Math.round(maxData / 100) * 100) / 5
  );
